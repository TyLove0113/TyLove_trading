...
網頁右上角有「🥇 黃金」連結，會去 `/gold` 頁面，分析 XAU/USD。
用另一個 Telegram bot 推送，唔會同港股訊息撈亂。

### 一定要開第二個 Telegram bot
1. Telegram 搵 **@BotFather** → 打 `/newbot`
2. 改名（例如 `TyLove Gold`）→ 改 username（例如 `tylove_gold_bot`）
3. BotFather 會俾你一串 token → 填入 `TELEGRAM_GOLD_BOT_TOKEN`
4. 搵 **@userinfobot** → 打任何訊息 → 佢會回你 `Id: 123456789` → 填入 `TELEGRAM_GOLD_CHAT_ID`
5. **重要**：要先去搵你新開嘅 bot，按 Start 講句嘢，否則 bot 唔可以主動發訊息俾你
6. 喺 `/gold` 頁按「測試黃金 bot」確認通

### 時區（重要修正）
之前通知時間全錯，係因為程式用嘅排程套件跟「容器時間」（Railway = UTC），
實際慢／快 8 小時。v3 已改成自己計香港時間，唔再受容器影響。
另外建議喺 Railway 加 `TZ=Asia/Hong_Kong` 做雙重保險。

### ⚠️ 誠實聲明
黃金策略（Donchian 20 突破、1.0×ATR 止蝕、3.0×ATR 目標）經兩年樣本外測試後
**利潤因子由 2.17 跌到 0.90（即係蝕錢）**。呢個策略未證實有穩定優勢，
只適合練習同記錄，唔應該視為賺錢保證。

### 記錄黃金交易
去「💼 持倉管理」，代號填 `XAUUSD`，**股數一欄填盎司數**
（0.01 手 = 1 盎司、0.05 手 = 5 盎司），咁賺蝕金額就等於你嘅美元盈虧。
